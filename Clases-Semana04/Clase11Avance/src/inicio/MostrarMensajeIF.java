...
public class MostrarMensajeIF extends JFrame {
	...
	public static void mostrarTabla(String autor, JTable tabla) {
	    String query = "SELECT * FROM mensajes WHERE autor_mensajes = '" + autor + "'";
	...
	private void llenarComboBox(JComboBox<String> comboBox) {
	    String query = "SELECT DISTINCT autor_mensajes FROM mensajes";
	    try (Connection conn = ConnectDatabase.getConnection();
	         Statement stmt = conn.createStatement();
	         ResultSet rs = stmt.executeQuery(query)) {

	        while (rs.next()) {
	            String nombre = rs.getString("autor_mensajes");
